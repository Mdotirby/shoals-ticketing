...
export default function AdminCreateEventPage() {
  ...
  /**
   * Promo codes and tracking links, staged before the show exists.
   *
   * Neither can be written until the event has an id, so they are collected
   * here and created straight after the POST. The page used to say "save this
   * event first, then add promo codes from the Edit page", which is a to-do
   * list handed to the operator at the exact moment they were already doing
   * the task.
   */
  const [promoDrafts, setPromoDrafts] = useState<
    { code: string; discount_type: "fixed" | "percentage"; discount_value: string; max_uses: string }[]
  >([]);
  const [promoDraft, setPromoDraft] = useState({ code: "", discount_type: "percentage" as "fixed" | "percentage", discount_value: "", max_uses: "" });
  /** Three links are generated by default — the channels a show actually gets shared on. */
  const [makeLinks, setMakeLinks] = useState(true);
  const [linkDrafts, setLinkDrafts] = useState([
    { label: "Instagram", source: "instagram", medium: "social" },
    { label: "Facebook", source: "facebook", medium: "social" },
    { label: "Email list", source: "email", medium: "email" },
  ]);
  /** What the created links ended up as, shown on the success screen. */
  const [createdLinks, setCreatedLinks] = useState<{ label: string; url: string }[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Both need the event id, so they are created here rather than asking
      // the operator to come back and do it from the Edit page. Best-effort:
      // the show exists either way, and anything that fails can be added
      // later from Promo & tracking.
      if (isHardTicket && event.id) {
        for (const p of promoDrafts) {
          try {
          ...
          } catch { /* add it from Promo & tracking */ }
        }

        if (makeLinks) {
          // The slug is what appears in the shared URL, so it is built from
          // the channel and a short random tail — readable, and unique
          // without a round trip to check.
          const base = (form.title || "show").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 18);
          const made: { label: string; url: string }[] = [];
          for (const l of linkDrafts) {
            const label = l.label.trim();
            if (!label) continue;
            const channel = (l.source || label).toLowerCase().replace(/[^a-z0-9]+/g, "-").slice(0, 12);
            const slug = `${base}-${channel}-${Math.random().toString(36).slice(2, 6)}`;
            try {
              const r = await fetch(`/api/events/${event.id}/trackable-links`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ label, slug, source: l.source || null, medium: l.medium || null }),
              });
              if (r.ok) {
                const created = await r.json();
                made.push({ label, url: created?.destination_url ? `${window.location.origin}/t/${slug}` : `${window.location.origin}/t/${slug}` });
              }
            } catch { /* add it from Promo & tracking */ }
          }
          if (made.length) setCreatedLinks(made);
        }
      }
  ...
  return (
    <div className="admin-form-page cshow">
      ...
      <div className="cshow-layout">
        ...
        {/* ── Promo codes, set up here rather than "later" ── */}
        {isHardTicket && (
          <div className="cshow-card" style={{ marginTop: 8 }}>
            <div className="cshow-eyebrow">Promo codes</div>
            <p className="cshow-note" style={{ marginTop: 6 }}>
              Created with the show. A code is checked at checkout against this event only.
            </p>

            {promoDrafts.length > 0 && (
              <div className="mkt-draft-list">
                {promoDrafts.map((p, i) => (
                  <div key={i} className="mkt-draft">
                    <strong>{p.code}</strong>
                    <span>
                      {p.discount_type === "percentage" ? `${p.discount_value}% off` : `$${p.discount_value} off`}
                      {p.max_uses ? ` · ${p.max_uses} uses` : " · unlimited"}
                    </span>
                    <button type="button" onClick={() => setPromoDrafts((prev) => prev.filter((_, n) => n !== i))} aria-label={`Remove ${p.code}`}>✕</button>
                  </div>
                ))}
              </div>
            )}

            <div className="mkt-promo-row">
              <label>
                <span>Code</span>
                <input
                  value={promoDraft.code}
                  onChange={(e) => setPromoDraft({ ...promoDraft, code: e.target.value.toUpperCase().replace(/\s+/g, "") })}
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) => setPromoDraft({ ...promoDraft, discount_type: e.target.value as "fixed" | "percentage" })}
                >
              ...
              <label>
                <span>Amount</span>
                <input type="number" min="0" step="0.01" value={promoDraft.discount_value} onChange={(e) => setPromoDraft({ ...promoDraft, discount_value: e.target.value })} />
              </label>
              <label>
                <span>Max uses</span>
                <input type="number" min="0" value={promoDraft.max_uses} onChange={(e) => setPromoDraft({ ...promoDraft, max_uses: e.target.value })} placeholder="∞" />
              </label>
              <button
                type="button"
                className="cshow-btn"
                disabled={!promoDraft.code.trim() || !promoDraft.discount_value}
                onClick={() => {
                  setPromoDrafts((prev) => [...prev, { ...promoDraft, code: promoDraft.code.trim() }]);
                  setPromoDraft({ code: "", discount_type: "percentage", discount_value: "", max_uses: "" });
                }}
              >
                Add
              </button>
        ...
        {isHardTicket && (
          <div className="cshow-card" style={{ marginTop: 8 }}>
            <div className="cshow-card-head">
              <div className="cshow-eyebrow">Tracking links</div>
              <span className="cshow-spacer" />
              <label className="mkt-toggle">
                <input type="checkbox" checked={makeLinks} onChange={(e) => setMakeLinks(e.target.checked)} />
                <span>Generate with the show</span>
              </label>
            </div>
            <p className="cshow-note" style={{ marginTop: 6 }}>
              One short link per channel, so you can tell which one sold the ticket. Each records
              views, clicks, time on the page, and a conversion — counted only when a payment
              succeeds and an order exists, never on a click.
            </p>

            {makeLinks && (
              <div className="mkt-draft-list">
                {linkDrafts.map((l, i) => (
                  <div key={i} className="mkt-draft mkt-draft--link">
                    <input
                      value={l.label}
                      onChange={(e) => setLinkDrafts((prev) => prev.map((x, n) => (n === i ? { ...x, label: e.target.value } : x)))}
                      aria-label="Link name"
                    />
                    <input
                      value={l.source}
                      onChange={(e) => setLinkDrafts((prev) => prev.map((x, n) => (n === i ? { ...x, source: e.target.value } : x)))}
                      aria-label="Source"
                      placeholder="source"
                    />
                    <button type="button" onClick={() => setLinkDrafts((prev) => prev.filter((_, n) => n !== i))} aria-label={`Remove ${l.label}`}>✕</button>
                  </div>
                ))}
                {linkDrafts.length < 8 && (
                  <button type="button" className="cshow-btn" onClick={() => setLinkDrafts((prev) => [...prev, { label: "", source: "", medium: "other" }])}>
                    + Another channel
                  </button>
                )}
              </div>
