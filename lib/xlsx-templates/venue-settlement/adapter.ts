 ...
 *   - bonus / rebate_to_promoter: settlement.ticketing_fees *
 *     settlement.service_fee_rebate_pct -- the real, current mechanism for
 *     "hand back part of the service fee as a deal term" (see
 *     lib/settlement/model.ts's artistPayout()). NOT settlement.ticketing_rebate,
 *     which is a separate, unrelated legacy field (misc venue-side ticketing
 *     platform revenue) that never reaches the artist.
 *   - balance_due already has this rebate folded in (artistPayout()'s
 *     artistTotal = guarantee + artistBackend + serviceFeeRebate, and
 *     balance_due is derived from artistTotal) -- so total_artist_walkout
 *     does NOT add bonus a second time on top of balance_due, unlike the
 *     source spreadsheet's own formula, which keeps them separate because
 *     its "Total Due to Artist" line doesn't include the rebate the way the
 *     real app's balance_due does.
 ...
 *     calculates) rather than re-deriving the sheet's own
 *     GBOR+Rebate+CashSales+MerchToVenue chain -- guarantees the
 *     bottom-line P&L always matches the app. Since the rebate was never
 *     added to venue revenue in the first place (unlike the old
 *     ticketing_rebate field), there's no double-count to unwind here.
...
export function buildVenueSettlementData(
  ...
  const expenses_variable = [
    ...expenses
      ...
      .map((e) => ({ name: e.name, amount: e.actual_amount || 0 })),
    // The card surcharge as the show expense it is. Derived from stored
    // cc_fees, because it is whatever the orders actually carried.
    // settlements.net_receipts is FACE value with the surcharge already out,
    // so it is added back into nbor below -- the two cancel and the pool is
    // unchanged, which keeps finalised settlements paying what they paid.
    ...(cardSurcharge > 0 ? [{ name: SETTLEMENT_CARD_EXPENSE_NAME, amount: cardSurcharge }] : []),
  ];
  ...
  const bonus = (settlement.ticketing_fees || 0) * (settlement.service_fee_rebate_pct || 0);
  ...
  return {
    ...
    balance_due,
    // balance_due already includes the service-fee rebate (via artistTotal
    // in artistPayout()) -- don't add bonus again here.
    total_artist_walkout: deposits_paid + cash_advance + balance_due,
