 ...
 * app's own already-computed, already-tested fields -- see
 * app/admin/settlements/[id]/page.tsx for where they're calculated and
 * saved) or derived with the exact same formulas as
 * lib/pdf-templates/settlement-report/adapter.ts. This function introduces
 * no new settlement math -- the Excel design's own formulas were used only
 * to find WHICH cell each value belongs in, not to decide what the value
 * IS. Two known places where they'd disagree if we'd copied the sheet's
 * formulas literally:
 *   - "Backend Overage": the sheet computes (Splitpoint × Backend%) − Guarantee;
 *     the app computes (NetAfterExpenses − Guarantee) × Backend%, floored at 0.
 *     Different numbers. We use settlement.artist_backend (the app's real value).
 *   - "Total Due to Artist": confirmed with Matt to include Backend Overage
 *     (matches VENUE_SETTLEMENTv2's formula, not v1's). settlement.balance_due
 *     already does this correctly (see the ticketing_rebate fix in the same
 *     session this adapter was built).
 */
