...
export async function exportContractPDF(
  ...
  doc.setFontSize(22);
  doc.setTextColor(GOLD);
  doc.text("PERFORMANCE AGREEMENT", MARGIN, 22);

  // Venue letterhead
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.setTextColor(WHITE);
  doc.text(venueName, MARGIN, 33);
  if (venueAddr) doc.text(venueAddr, MARGIN, 39);
  const contactParts = [venue.buyer_phone, venue.buyer_email].filter(Boolean);
  if (contactParts.length) doc.text(contactParts.join("  |  "), MARGIN, 45);

  // Date (right)
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.setTextColor(GOLD);
  doc.text(`Date: ${eventDate}`, MARGIN + CONTENT_W, 33, { align: "right" });
  ...
  if (offer.fixed_expenses?.length || offer.variable_expenses?.length) {
    ...
    if (offer.fixed_expenses?.length) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(8);
      y = ensureSpace(doc, 7, y);
      ...
      y += 7;
      doc.setFont("helvetica", "normal");
      for (const e of offer.fixed_expenses) {
        y = ensureSpace(doc, 6, y);
        doc.text(e.name, MARGIN + 8, y + 4);
    ...
    if (offer.variable_expenses?.length) {
      doc.setFont("helvetica", "bold");
      doc.setFontSize(8);
      y = ensureSpace(doc, 7, y);
      doc.text("Variable Expenses:", MARGIN + 3, y + 4);
      y += 7;
      doc.setFont("helvetica", "normal");
      for (const e of offer.variable_expenses) {
        y = ensureSpace(doc, 6, y);
        doc.text(`${e.name} (${(e.rate * 100).toFixed(2)}%)`, MARGIN + 8, y + 4);
        doc.text(fmt(e.amount), MARGIN + CONTENT_W - 3, y + 4, { align: "right" });
        y += 6;
      }
      y += 2;
