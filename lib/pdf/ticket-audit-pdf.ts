 ...
 */
import {
  loadVenueLogo, loadVenueCoreFavicon,
...
export async function generateTicketAuditPDF(data: AuditData): Promise<ArrayBuffer> {
  ...
  const vcIconDataUrl = await loadVenueCoreFavicon();
  ...
  for (let p = 1; p <= totalPages; p++) {
    doc.setPage(p);
    drawLandscapeFooter(doc, p, totalPages, vcIconDataUrl);
  }
...
function drawLandscapeFooter(doc: Doc, page: number, total: number, vcIconDataUrl?: string | null) {
  const y = PAGE_H - 8;
  ...
  const footerText = "Generated by VenueCore  •  Ticket Audit — Confidential";
  if (vcIconDataUrl) {
    try {
      const iconSize = 3.5;
      doc.addImage(vcIconDataUrl, "PNG", MARGIN, y - 0.5, iconSize, iconSize);
      doc.text(footerText, MARGIN + iconSize + 1.5, y + 3);
    } catch {
      doc.text(footerText, MARGIN, y + 3);
    }
  } else {
    doc.text(footerText, MARGIN, y + 3);
  }
