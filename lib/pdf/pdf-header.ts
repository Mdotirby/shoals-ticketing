...
export const DARK_W72: [number, number, number] = [0, 0, 0];
export const WHITE: [number, number, number] = [255, 255, 255];

/** Detect which operator is hosting this page — used for PDF color selection only */
function getPdfOperator(): "west72" | "venuecore" {
  if (typeof window === "undefined") return "venuecore";
  return window.location.hostname.includes("west72") ? "west72" : "venuecore";
}
...
export function drawFooter(doc: Doc, documentTitle?: string, opts?: { vcIconDataUrl?: string }) {
  ...
  for (let i = 1; i <= pages; i++) {
    ...
    const platform = getPdfOperator() === "west72" ? "West 72 Entertainment" : "VenueCore";
    const footerText = `${label}Generated by ${platform}  •  Page ${i} of ${pages}`;

    // VenueCore favicon to the left of footer text
    if (opts?.vcIconDataUrl) {
      try {
        const iconSize = 3.5; // mm — tiny
        const textW = doc.getTextWidth(footerText);
        const textStartX = (PAGE_WIDTH - textW) / 2;
        doc.addImage(opts.vcIconDataUrl, "PNG", textStartX - iconSize - 1.5, FOOTER_Y - iconSize + 0.5, iconSize, iconSize);
      } catch { /* skip icon on error */ }
    }

    doc.text(footerText, PAGE_WIDTH / 2, FOOTER_Y, { align: "center" });
...
/**
 * Load the VenueCore platform icon for use in PDF footers.
 * Tries /vc-icon.png, /venuecore-icon.png, /favicon.png in order.
 */
export async function loadVenueCoreFavicon(): Promise<string | null> {
  if (typeof window === "undefined") return null;
  const origin = window.location.origin;
  for (const path of ["/vc-icon.png", "/venuecore-icon.png", "/favicon.png", "/apple-icon.png"]) {
    const result = await loadLogoAsDataURL(`${origin}${path}`);
