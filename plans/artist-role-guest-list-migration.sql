-- Migration: artist role + guest list tables
...
-- 2. Ensure must_change_password column exists (default true for new users)
...
-- Organizer/owner can manage assignments
...
-- Artist can read their own assignments
...
-- Artist can manage their own guest list entries
CREATE POLICY "artist_manage_own_guests" ON guest_list
  FOR ALL USING (artist_id = auth.uid());

-- Organizer/owner can read all guest lists for their venue events
